...
- [ ] **TF build verification checklist** (2026-05-22) — production build (1.0.0 build 23) was kicked at the end of session 2026-05-22 via `npx eas-cli build --platform ios --profile production --auto-submit`. ASC API key auth used (chris.chidgey@protonmail.com is Admin of Apple Team 82V5W9KUUD per ASC; EAS team picker didn't show the team initially but the ASC API key + EXPO_APPLE_TEAM_ID env var let the build proceed). Auto-submits to ASC TestFlight when done.
  - [ ] **IAP purchase loop (the big one)** — subscribe with a sandbox Apple ID → confirm entitlement reaches RC → `useIsPlus()` returns true on next app open → paywall closes / Plus features unlock. Restore Purchases also works. Sandbox tester is created in ASC → Users and Access → Sandbox Testers (or use existing if one's set up).
  - [ ] **Sentry DSN in prod** — never verified at runtime, only the build-time sourcemap upload. Force a crash (temp debug button or `throw` in a reachable code path) and confirm event lands in Sentry → gymcrush org → gym-crush project within ~1 min.
  - [ ] **Push notifications on physical device** — preview build smoke had pushes no-op'd on simulator. Now on real device: trigger a match (Block 1/2/3 test SQL still works) and confirm the push arrives, deep-links correctly to chat.
  - [ ] **Block flow** — needs 2 accounts on 2 devices (or sim + phone). User A blocks B → A vanishes from B's discover, B can't message/like A.
  - [ ] **Bug 2 — gender filter not applying** (from preview smoke). Open Discovery Preferences → Select "Women" → close. Does discover feed actually filter to women only? Repro carefully: paste back which UI shows "Everyone" — the Select dropdown text? The discover feed contents? Did Save get tapped? Code path traced 3 layers deep in DiscoveryPreferences.tsx + discover.tsx; nothing in data flow drops the change.
  - [ ] **Bug 3 — offerwall bottom sheet cut off** (from preview smoke). The "Buy" button is partially obscured at the bottom of the paywall modal. Likely a `snapPoints` issue in `OfferWallModal.tsx` — either bump it to ~95% or add safe-area inset padding to the action row. UI-only fix, no DB or RPC dependency.
  - [ ] **Bug 4 — gym search "no gyms found"** (from preview smoke). Type a gym name in onboarding or Edit Profile → autocomplete returns nothing. Could be (a) the `X-Ios-Bundle-Identifier` header isn't being sent in production bundle the same way as dev (verify in Charles or via a Sentry breadcrumb), or (b) the GCP Places API key restriction is over-strict (e.g. only allowing specific bundle in a specific signing format). If reproducing on TF, test from a fresh terminal: temporarily switch GCP key restrictions to "None" → see if it works → if yes, narrow restriction.
  - [ ] **Discovery Preferences slider stale on first open (bug 1)** — fixed in commit `7d75164` by passing `currentMaxDistanceMiles` as live prop. Re-verify in TF: move top-level distance slider in Discover → open prefs modal → modal should show the same value.

  After all of the above pass, send TestFlight invite to the client as an Internal Tester (ASC → TestFlight → Internal Testing → add chris.chidgey@toptal.com... wait, the *client*'s email).
